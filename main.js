...
const changeDeckCheckbox = document.getElementById("changeDeckCheckbox");const frontInputField = document.getElementById("frontInputField");
...
function displayOpenAnki() {
    let htmlToLoad = "<span style=\"color: red; font-weight: bold;\">Please open Anki locally and reopen this extension to save new cards</span>";
...
function fetchDecks(selectElement) {
  invoke('deckNames', 23)
      .then((result) => {
        ...
        selectElement.innerHTML += "<option value=\"" + result[i] + "\">" + result[i] +  "</option>";
        }
